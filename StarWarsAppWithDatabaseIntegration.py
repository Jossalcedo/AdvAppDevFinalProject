...
class StarWarsApp:
    ...
    def create_main_frame(self):
        ...
        title_label = ctk.CTkLabel(main_frame, text="Welcome to the Star Wars Rebellion App", font=("Helvetica", 24, "bold"), text_color="yellow")
    ...
    def create_planet_pages(self):
        # Adding additional information using kwargs. I added this after I did the code in order to add some kwargs into the code.
    ...
    #to show the current time on planet
    ...
    def save_message(self, **kwargs):
        ...
        with sqlite3.connect("star_wars_app.db") as conn:
            cursor = conn.cursor()
            cursor.execute("CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY, base TEXT, message TEXT)")
            cursor.execute("INSERT INTO messages (base, message) VALUES (?, ?)", (base, message))
            conn.commit()
    ...
    def check_login(self):
        ...
        password = self.password_entry.get()
                                                #**********************THE ONLY ACCOUNT ON THE SYSTEM*************************
    ...
    def update_messages(self):
        ...
        with sqlite3.connect("star_wars_app.db") as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT base, message FROM messages")
            for row in cursor.fetchall():
                self.messages_listbox.insert(tk.END, f"{row[0]}: {row[1]}")
